...
import { isBinaryFile } from "./binary-content-detection.js";
...
function createSandboxReadOperations(params: SandboxToolParams) {
  return {
    readFile: async (absolutePath: string) => {
      ...
      // Binary content detection: prevent corrupted text output for binary files
      if (isBinaryFile(buffer)) {
        throw new Error(
          `Refusing to read binary file as text: ${absolutePath}. Use the image tool or check the file directly.`
        );
      }
