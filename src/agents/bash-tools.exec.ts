...
import { verifyBashCommand } from "./bash-verification.js";
import { verifySedCommand, isSedCommand } from "./sed-validation.js";
import { checkImmediateInstincts, emitErrorSignal } from "../autonomous/instinct/instinct-system.js";
...
// Command classification constants (inspired by Claude Code's BashTool)
...
export const BASH_LIST_COMMANDS = new Set(["ls", "tree", "du", "fd", "find"]);
...
// Commands that are destructive (modify or delete data)
// These should be flagged for security review in auto-mode
...
/**
 * Extract the primary executable name from a shell command string.
 * Handles env prefixes, sudo, shell wrappers (bash -c), etc.
 */
function extractPrimaryCommand(rawCommand: string): string | null {
  ...
  // Skip shell wrappers (bash, sh, zsh, etc. with -c)
  const shellWrappers = new Set(["bash", "dash", "fish", "ksh", "sh", "zsh"]);
  while (idx < argv.length && shellWrappers.has(argv[idx])) {
    idx += 1;
    // Skip the -c flag and get the next argument as the actual command
    if (argv[idx] === "-c" || (argv[idx] && argv[idx].startsWith("-c"))) {
      idx += 1;
      // The next token is the script payload - don't try to parse further
      break;
    }
  }
  ...
  // Strip path components, leaving just the command name
  const base = primary.split(/[\\/]/u).at(-1)?.toLowerCase() ?? "";
...
/**
 * Classify a shell command by its primary executable.
 */
export function classifyBashCommand(command: string): BashCommandCategory {
  ...
  if (BASH_SEARCH_COMMANDS.has(primary)) {
    return { kind: "search" };
  }
  if (BASH_READ_COMMANDS.has(primary)) {
    return { kind: "read" };
  }
  if (BASH_LIST_COMMANDS.has(primary)) {
    return { kind: "list" };
  }
  if (BASH_SILENT_COMMANDS.has(primary)) {
    return { kind: "silent" };
  }
...
/** Returns true if the command is a search or read operation. */
export function isSearchOrReadCommand(command: string): boolean {
  const cat = classifyBashCommand(command);
  return cat.kind === "search" || cat.kind === "read";
}

/** Returns true if the command is a search operation (find, grep, etc.). */
export function isSearchCommand(command: string): boolean {
  return classifyBashCommand(command).kind === "search";
}

/** Returns true if the command is a read operation (cat, head, etc.). */
export function isReadCommand(command: string): boolean {
  return classifyBashCommand(command).kind === "read";
}

/** Returns true if the command is a list operation (ls, tree, etc.). */
export function isListCommand(command: string): boolean {
  return classifyBashCommand(command).kind === "list";
}

/** Returns true if the command is a silent operation (mv, cp, rm, etc.). */
export function isSilentCommand(command: string): boolean {
  return classifyBashCommand(command).kind === "silent";
}

/** Returns true if the command is destructive (rm, dd, mkfs, etc.). */
 ...
 *
 * Returns '' for safe commands (skipped in classifier).
 * Returns the command itself for destructive/risky commands (needs review).
 */
export function toAutoClassifierInput(command: string): string {
  const category = classifyBashCommand(command);
  // Only include destructive commands in the security classifier transcript
  if (category.kind === "destructive") {
    return command;
  }
  // Skip silent/semantic-neutral commands - they have no security relevance
...
/**
 * Build a human-readable description of what a command does based on its classification.
 */
export function describeCommandCategory(command: string): string {
  const cat = classifyBashCommand(command);
  switch (cat.kind) {
    case "search":
      return "Search operation (find, grep, ripgrep, etc.) — scans files/directories";
    case "read":
      return "Read operation (cat, head, tail, jq, etc.) — reads file contents";
    case "list":
...
function buildExecForegroundResult(params: {
  ...
  // Build result prefix and stats based on command classification
  let resultPrefix = "";
  let resultStats = "";

  if (command) {
    const category = classifyBashCommand(command);
    const output = params.outcome.aggregated || "";

    switch (category.kind) {
      case "search":
        resultPrefix = "[search] ";
        // Count grep/rg/fd matches: each line is typically one match
        if (output) {
          const lineCount = output.split("\n").length;
          resultStats = lineCount > 1 ? ` (${lineCount} matches)` : " (1 match)";
        }
        break;
      case "read":
        resultPrefix = "[read] ";
        // Show line count for read commands (cat, head, wc -l, etc.)
        if (output) {
          const lineCount = output.split("\n").length;
          const wordCount = output.split(/\s+/).filter(Boolean).length;
          // If this looks like a file read (many lines or has content), show stats
          if (lineCount > 3) {
            resultStats = ` (${lineCount} lines, ${wordCount} words)`;
          } else if (lineCount === 1 && wordCount > 50) {
            resultStats = ` (${wordCount} words)`;
          }
        }
        break;
      case "list":
        resultPrefix = "[list] ";
        // Count directory entries for ls commands
        if (output) {
          const lines = output.split("\n").filter(Boolean);
          const entryCount = lines.length;
          resultStats = entryCount > 0 ? ` (${entryCount} entries)` : "";
        }
        break;
      case "silent":
        // Silent commands (mv, cp, rm, etc.) show exit code on failure only
        resultPrefix = "";
        break;
      case "destructive":
  ...
  const output = params.outcome.aggregated || "(no output)";
  const displayOutput =
    resultPrefix && output === "(no output)"
      ? `${resultPrefix}(no output)`
      : `${resultPrefix}${output}${resultStats}`;

  return textResult(`${warningText}${displayOutput}`, {
    status: "completed",
 ...
 * Based on Claude Code's BashTool.validateInput pattern.
 * First phase of two-phase permission checking:
 * 1. validateInput (sync, fast) - checks format/completeness
 * 2. checkPermissions (async) - user approval, dangerous patterns
 */
function validateExecInput(input: Record<string, unknown>): {
  valid: boolean;
  error?: string;
  errorCode?: number;
} {
  ...
  if (!command) {
    return { valid: false, error: "exec: missing required parameter 'command'", errorCode: 1 };
  }

  if (typeof command !== "string") {
    return { valid: false, error: "exec: 'command' must be a string", errorCode: 1 };
  }

  if (command.length > MAX_COMMAND_LENGTH) {
    return {
      valid: false,
      error: `exec: command exceeds maximum length of ${MAX_COMMAND_LENGTH.toLocaleString()} characters`,
      errorCode: 2,
    };
  }

  if (!command.trim()) {
    return { valid: false, error: "exec: empty command", errorCode: 1 };
  }

  // Block recursive sudo patterns
  if (/sudo\s+su\s/.test(command)) {
    return { valid: false, error: "exec: blocked recursive sudo pattern (sudo su)", errorCode: 3 };
  }
...
function buildExecToolDescription(agentId?: string): string {
  const base =
    ...
    "Command categories (results are annotated with type):\n" +
    "  Search (grep, find, rg, ag...): Annotated [search] with match count. For many results, prefer `grep -c` or `rg --count` to get exact counts.\n" +
    "  Read (cat, head, tail, less...): Annotated [read] with line/word count. Use `head`/`tail` for previews.\n" +
    "  List (ls, tree, du, fd...): Annotated [list] with entry count.\n" +
    "  Silent (mv, cp, rm, chmod...): No annotation on success. These commands produce no output when working correctly.\n\n" +
    "Tips:\n" +
    "  - For piped commands, all segments are analyzed; mixed pipes (search + destructive) may skip annotation.\n" +
    "  - Use `grep -c` / `rg -c` when you need exact match counts in large codebases.\n" +
    "  - Silent command failures are still reported with exit code.";
...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      const validation = validateExecInput(params as Record<string, unknown>);
      if (!validation.valid) {
      ...
      // Security verification: run all bash security validators
      const securityCheck = verifyBashCommand(params.command);
      if (securityCheck.behavior === 'deny') {
        throw new Error(
          `Command blocked by security check: ${securityCheck.warnings.join('; ')}`
        );
      }
      if (securityCheck.behavior === 'ask') {
        warnings.push(`Security warning: ${securityCheck.warnings.join('; ')}`);
      }

      // Dedicated sed command validation (if this is a sed command)
      if (isSedCommand(params.command)) {
        const sedCheck = verifySedCommand(params.command);
        if (sedCheck.behavior === 'deny') {
          throw new Error(
            `sed command blocked: ${sedCheck.warnings.join('; ')}`
          );
        }
        if (sedCheck.behavior === 'ask') {
          warnings.push(`sed security: ${sedCheck.warnings.join('; ')}`);
        }
      }

      // Instinct system: check critical instincts before execution
      const instinctResults = checkImmediateInstincts({
      ...
      for (const result of instinctResults) {
        if (result.action?.type === 'safety_warning' && result.action.immediate) {
          throw new Error(
            `Command blocked by instinct system: ${result.reason}`
          );
        }
        if (result.priority >= 80) {
          warnings.push(`Instinct: ${result.reason}`);
        }
      }
      ...
      return new Promise<AgentToolResult<ExecToolDetails>>((resolve, reject) => {
        ...
        run.promise
          ...
          .catch((err) => {
            ...
            // Emit error signal to instinct system for learning
            emitErrorSignal(err as Error, { command: params.command, sessionKey: defaults?.sessionKey });
            reject(err as Error);
          });
      });
    },
